...
BETA2 = 0.01  # demand-driven vol shock
BETA3 = 0.05  # event-driven vol spike (only negative events — see market_state.py)

# ---- VALUE ANCHOR (EWMA) ----
# FIX: EWMA_ALPHA was hardcoded in simulator.py.
# Moved here so it can be tuned alongside all other parameters.
# Lower alpha = longer memory = EWMA drifts more slowly toward crash prices.
# At alpha=0.05: after 30 steps of crash, EWMA retains ~78% of pre-crash value.
EWMA_ALPHA = 0.05
...
EVENT_AT = {
    5:  "strong_positive",   # e.g., surprise earnings beat, central bank rate cut
}
